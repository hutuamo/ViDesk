...
# FreeRDP for visionOS Device 编译脚本

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
FREERDP_SRC="${SCRIPT_DIR}/FreeRDP"
BUILD_DIR="${SCRIPT_DIR}/build-freerdp-device"
OUTPUT_DIR="${SCRIPT_DIR}/output/freerdp-device"
OPENSSL_DIR="${SCRIPT_DIR}/output/openssl-device"

# visionOS Device SDK
XROS_SDK=$(xcrun --sdk xros --show-sdk-path)
TARGET="arm64-apple-xros2.0"

echo "=== 编译 FreeRDP for visionOS Device ==="
echo "SDK: ${XROS_SDK}"
echo "OpenSSL: ${OPENSSL_DIR}"

if [ ! -d "${FREERDP_SRC}" ]; then
    echo "错误: FreeRDP 源码不存在: ${FREERDP_SRC}"
    echo "请先克隆 FreeRDP:"
    echo "  git clone https://github.com/FreeRDP/FreeRDP.git ${FREERDP_SRC}"
    exit 1
fi
...
if [ ! -f "${OPENSSL_DIR}/lib/libssl.a" ]; then
    echo "错误: OpenSSL 库不存在"
    echo "请先运行: ./build-openssl-visionos-device.sh"
    exit 1
fi
...
echo ""
echo "=== FreeRDP for visionOS Device 编译结果 ==="
ls -la "${PROJECT_LIB}"

echo ""
echo "验证库架构..."
lipo -info "${PROJECT_LIB}/libfreerdp3.a" 2>/dev/null || true
